...
/***/function (module, exports, __webpack_require__) {
	...
	var ChatClient = __webpack_require__(54);
		...
		socket.on('leaderboard', function (data) {
			...
			for (var i = 0; i < leaderboard.length; i++) {
				...
				if (leaderboard[i].id == player.id) {
					if (leaderboard[i].name.length !== 0) status += '<span class="me">' + (i + 1) + '. ' + leaderboard[i].name + " " + leaderboard[i].mass + " ETHOS </span>";else status += '<span class="me">' + (i + 1) + ". An unnamed cell " + leaderboard[i].mass + "  ETHOS</span>";
				} else {
					if (leaderboard[i].name.length !== 0) status += i + 1 + '. ' + leaderboard[i].name + " " + leaderboard[i].mass;else status += i + 1 + '. An unnamed cell' + " " + leaderboard[i].mass;
				}
			}
			console.log(leaderboard);
			//status += '<br />Players: ' + data.players;
			//document.getElementById('status').innerHTML = "<span>" + status;
			document.getElementById('status').innerHTML = status;
		});

		//  setInterval(function(){document.getElementById('status').innerHTML = document.getElementById('status').innerHTML;}, 1);
